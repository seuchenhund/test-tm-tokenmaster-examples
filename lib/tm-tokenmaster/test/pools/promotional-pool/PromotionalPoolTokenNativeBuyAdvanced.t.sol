...
contract PromotionalPoolTokenNativeBuyAdvancedTest is PromotionalPoolTokenTest {
    ...
    function testRevertsBuyHookWhenOrderIsDisabled() public {
        ...
        PromotionalPool(tmps.tokenAddress).grantRole(ORDER_MANAGER_ROLE, manager);
        vm.stopPrank();
    ...
    function testSpendHook() public {
        ...
        tmps.tokenAddress = _deployToken(tmps.deployer, deploymentParameters, emptySignature, NO_ERROR);
        _dealPaired(tmps.trader, 10 ether);
        _buyTokens(tmps.traderKey, tmps.trader, PromotionalPool(tmps.tokenAddress), 5 ether, 6 ether, 6 ether, block.timestamp, NO_ERROR);
