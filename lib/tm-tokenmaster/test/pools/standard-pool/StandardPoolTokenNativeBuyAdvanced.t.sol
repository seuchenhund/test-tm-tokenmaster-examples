...
contract StandardPoolTokenNativeBuyAdvancedTest is StandardPoolTokenTest {
    ...
    function testRevertsBuyHookWhenOrderIsDisabled() public {
        ...
        StandardPool(tmps.tokenAddress).grantRole(ORDER_MANAGER_ROLE, manager);
        vm.stopPrank();
    ...
    function testSellHook() public {
        ...
        tmps.tokenAddress = _deployToken(tmps.deployer, deploymentParameters, emptySignature, NO_ERROR);
        _dealPaired(tmps.trader, 10 ether);
        _buyTokens(tmps.traderKey, tmps.trader, StandardPool(tmps.tokenAddress), 5 ether, 6 ether, 6 ether, block.timestamp, NO_ERROR);
