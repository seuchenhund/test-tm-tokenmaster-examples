...
import "./StablePoolTokenHandler.sol";
import "src/TokenMasterRouter.sol";
import "src/pools/stable-pool/StablePoolFactory.sol";
import "src/pools/stable-pool/StablePoolCreationCode.sol";
...
contract StablePoolTokenInvariants is TokenMasterTest {
    BondedPool public pool;
    StablePoolTokenHandler public handler;
    ...
    function setUp() public override {
        ...
        DeploymentParameters memory deploymentParameters = DeploymentParameters({
            tokenFactory: address(stablePoolFactory),
            tokenSalt: poolSalt,
        ...
        address poolAddress = stablePoolFactory.computeDeploymentAddress(
        ...
        MockPairedTokenERC20(deploymentParameters.poolParams.pairedToken).mint(
            initialOwner, 
            deploymentParameters.poolParams.initialPairedTokenToDeposit
        );
        vm.startPrank(initialOwner);
        MockPairedTokenERC20(deploymentParameters.poolParams.pairedToken).approve(
        ...
        handler = new StablePoolTokenHandler(TokenMasterTest(address(this)), tokenMasterRouter, pool, deploymentParameters, initParams);
        ...
        selectors[0] = StablePoolTokenHandler.buyTokens.selector;
        selectors[1] = StablePoolTokenHandler.sellTokens.selector;
        selectors[2] = BondedPoolHandler.addSpendSigner.selector;
        selectors[3] = BondedPoolHandler.spendTokens.selector;
        selectors[4] = StablePoolTokenHandler.setBuyParameters.selector;
        selectors[5] = StablePoolTokenHandler.setSellParameters.selector;
        selectors[6] = BondedPoolHandler.withdrawCreatorShare.selector;
        selectors[7] = BondedPoolHandler.withdrawFees.selector;
        selectors[8] = BondedPoolHandler.approve.selector;
        selectors[9] = BondedPoolHandler.transfer.selector;
        selectors[10] = BondedPoolHandler.transferFrom.selector;
        selectors[11] = BondedPoolHandler.losePairedTokens.selector;
    ...
    // Scenario 0: Paired ERC20, matching 18 decimals
    // Scenario 1: Paired ERC20, matching random decimals
    // Scenario 2: Paired ERC20, mismatching random decimals
    function _getScenario(uint256 _seed) internal pure returns (uint256) {
        return uint256(_getRandomHash(_seed, 0)) % 3;
    }

    function _getPoolSalt(uint256 _seed) internal virtual pure returns (bytes32) {
        return _getRandomHash(_seed, 1);
    }

    function _getPoolOwner(uint256 _seed) internal virtual pure returns (address) {
    ...
    function _getPoolDeploymentParams(uint256 _seed, address initialOwner) internal virtual returns (PoolDeploymentParameters memory, StablePoolInitializationParameters memory) {
        (
        ...
        StablePoolInitializationParameters memory initializationParameters = StablePoolInitializationParameters({
            ...
            stablePairedPricePerToken: PairedPricePerToken({
                numerator: uint96(1 + (uint256(_getRandomHash(_seed, 25)) % (type(uint96).max - 1))),
                denominator: uint96(1 + (uint256(_getRandomHash(_seed, 25)) % (type(uint96).max - 1)))
            })
        });

        uint256 initialPairedTokenToDeposit = 
            initializationParameters.initialSupplyAmount * 
            initializationParameters.stablePairedPricePerToken.numerator / 
            initializationParameters.stablePairedPricePerToken.denominator;
    ...
    function _getDecimalsAndTokenAddressForScenario(
        uint256 scenario,
        address deployer
    ) internal returns (uint8 pairedTokenDecimals, uint8 poolTokenDecimals, address pairedToken) {
        ...
        if (scenario == 0) {
            ...
            pairedToken = address(new MockPairedTokenERC20("MockPairedToken", "MPT", 18));
            vm.stopPrank();
        } else if (scenario == 1) {
            pairedTokenDecimals = uint8(uint256(_getRandomHash(0, 5))) % 19;
            poolTokenDecimals = pairedTokenDecimals;
            vm.startPrank(deployer);
            pairedToken = address(new MockPairedTokenERC20("MockPairedToken", "MPT", pairedTokenDecimals));
            vm.stopPrank();
        } else if (scenario == 2) {
            pairedTokenDecimals = uint8(uint256(_getRandomHash(0, 6))) % 19;
            poolTokenDecimals = uint8(uint256(_getRandomHash(0, 7))) % 19;
            vm.startPrank(deployer);
            pairedToken = address(new MockPairedTokenERC20("MockPairedToken", "MPT", pairedTokenDecimals));
            vm.stopPrank();
        }
